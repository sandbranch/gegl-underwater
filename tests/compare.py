...
runs = sys.argv[1:]
width = 300
photos = sorted(glob.glob(os.path.join(here, 'images', '*%s*.jpg' % os.environ.get('ONLY', ''))))
for old in glob.glob(os.path.join(here, 'output', 'compare-*.jpg')):
    os.remove(old)
for k in range(0, len(photos), 8):
    rows = []
    for f in photos[k:k + 8]:
        name = os.path.basename(f)
        paths = [f] + [os.path.join(here, 'output', r, name) for r in runs]
    ...
    for r in rows:
        sheet.paste(r, (0, y))
        y += r.height
    out = os.path.join(here, 'output', 'compare-%d.jpg' % (k // 8))
